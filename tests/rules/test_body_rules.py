...
def test_body_empty_rule_passes_non_empty_with_never() -> None:
    rule = BodyEmptyRule()
    assert rule.validate(_commit("some body"), _config(RuleCondition.NEVER)) is None
...
def test_body_leading_blank_rule_passes_with_blank_line_and_always() -> None:
    ...
    raw = "feat: subject\n\nbody content"
    commit = CommitMessage(raw=raw, header="feat: subject", type="feat", subject="subject", body="body content")
...
def test_body_leading_blank_rule_skips_single_line_commit() -> None:
    rule = BodyLeadingBlankRule()
    commit = CommitMessage(raw="feat: subject", header="feat: subject", type="feat", subject="subject", body="x")
    assert rule.validate(commit, _config(RuleCondition.ALWAYS)) is None
...
def test_body_max_length_rule_passes_within_max() -> None:
    ...
    assert rule.validate(_commit("short body"), _config(RuleCondition.ALWAYS, value=100)) is None


def test_body_max_length_rule_fails_when_exceeds_max() -> None:
    rule = BodyMaxLengthRule()
    result = rule.validate(_commit("a very long body text"), _config(RuleCondition.ALWAYS, value=5))
    assert result is not None
...
def test_body_max_line_length_rule_passes_within_limit() -> None:
    ...
    assert rule.validate(_commit("short line"), _config(RuleCondition.ALWAYS, value=100)) is None


def test_body_max_line_length_rule_fails_on_long_line() -> None:
    rule = BodyMaxLineLengthRule()
    long_line = "x" * 120
    result = rule.validate(_commit(long_line), _config(RuleCondition.ALWAYS, value=100))
    assert result is not None
...
def test_body_max_line_length_rule_skips_url_lines() -> None:
    ...
    assert rule.validate(_commit(url_line), _config(RuleCondition.ALWAYS, value=80)) is None
...
def test_body_min_length_rule_passes_when_meets_min() -> None:
    ...
    assert rule.validate(_commit("enough body"), _config(RuleCondition.ALWAYS, value=5)) is None
...
def test_body_full_stop_rule_passes_without_period_with_never() -> None:
    ...
    assert rule.validate(_commit("body text"), _config(RuleCondition.NEVER, value=".")) is None


def test_body_full_stop_rule_fails_with_period_and_never() -> None:
    rule = BodyFullStopRule()
    result = rule.validate(_commit("body text."), _config(RuleCondition.NEVER, value="."))
    assert result is not None
...
def test_body_full_stop_rule_passes_with_period_and_always() -> None:
    rule = BodyFullStopRule()
    assert rule.validate(_commit("body text."), _config(RuleCondition.ALWAYS, value=".")) is None
...
@pytest.mark.parametrize("body,cases,condition,should_fail", [
...
def test_body_case_rule(body: str, cases: list, condition: RuleCondition, should_fail: bool) -> None:
    rule = BodyCaseRule()
