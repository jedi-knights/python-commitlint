...
def test_scope_empty_rule_passes_empty_with_always() -> None:
    rule = ScopeEmptyRule()
    assert (
        rule.validate(_commit_no_scope(), _config(RuleCondition.ALWAYS)) is None
    )
...
@pytest.mark.parametrize(
    "scope,case_value,condition,should_fail",
    [
...
def test_scope_case_rule(
    scope: str, case_value: str, condition: RuleCondition, should_fail: bool
) -> None:
    rule = ScopeCaseRule()
...
def test_scope_case_rule_skips_empty_scope() -> None:
    ...
    assert (
        rule.validate(
            _commit_no_scope(),
            _config(RuleCondition.ALWAYS, value="lower-case"),
        )
        is None
    )
...
def test_scope_min_length_rule_passes_when_meets_min() -> None:
    ...
    assert (
        rule.validate(_commit("api"), _config(RuleCondition.ALWAYS, value=2))
        is None
    )
...
def test_scope_min_length_rule_skips_empty_scope() -> None:
    ...
    assert (
        rule.validate(
            _commit_no_scope(), _config(RuleCondition.ALWAYS, value=5)
        )
        is None
    )
...
def test_scope_max_length_rule_passes_within_max() -> None:
    ...
    assert (
        rule.validate(_commit("api"), _config(RuleCondition.ALWAYS, value=10))
        is None
    )
...
def test_scope_max_length_rule_fails_when_exceeds_max() -> None:
    ...
    result = rule.validate(
        _commit("very-long-scope"), _config(RuleCondition.ALWAYS, value=5)
    )
    assert result is not None
    assert "5" in result.message
...
def test_scope_max_length_rule_skips_empty_scope() -> None:
    ...
    assert (
        rule.validate(
            _commit_no_scope(), _config(RuleCondition.ALWAYS, value=1)
        )
        is None
    )
