...
def test_subject_empty_rule_passes_when_non_empty_with_never() -> None:
    ...
    assert rule.validate(_commit("add feature"), _config(RuleCondition.NEVER)) is None
...
def test_subject_empty_rule_fails_when_non_empty_with_always() -> None:
    rule = SubjectEmptyRule()
    result = rule.validate(_commit("add feature"), _config(RuleCondition.ALWAYS))
    assert result is not None
...
@pytest.mark.parametrize("subject,cases,condition,should_fail", [
    ("add feature", ["sentence-case", "pascal-case"], RuleCondition.NEVER, False),
...
def test_subject_case_rule(subject: str, cases: list, condition: RuleCondition, should_fail: bool) -> None:
...
def test_subject_full_stop_rule_passes_without_period_with_never() -> None:
    ...
    assert rule.validate(_commit("add feature"), _config(RuleCondition.NEVER, value=".")) is None


def test_subject_full_stop_rule_fails_with_period_with_never() -> None:
    rule = SubjectFullStopRule()
    result = rule.validate(_commit("add feature."), _config(RuleCondition.NEVER, value="."))
    assert result is not None
...
def test_subject_full_stop_rule_passes_with_period_with_always() -> None:
    rule = SubjectFullStopRule()
    assert rule.validate(_commit("add feature."), _config(RuleCondition.ALWAYS, value=".")) is None


def test_subject_full_stop_rule_fails_without_period_with_always() -> None:
    rule = SubjectFullStopRule()
    result = rule.validate(_commit("add feature"), _config(RuleCondition.ALWAYS, value="."))
    assert result is not None


def test_subject_full_stop_rule_uses_default_period() -> None:
    rule = SubjectFullStopRule()
    result = rule.validate(_commit("add feature."), _config(RuleCondition.NEVER))
    assert result is not None
...
def test_subject_full_stop_rule_skips_empty_subject() -> None:
    rule = SubjectFullStopRule()
    commit = CommitMessage(raw="feat:", header="feat:", type="feat", subject="")
    assert rule.validate(commit, _config(RuleCondition.NEVER, value=".")) is None
...
def test_subject_min_length_rule_passes_when_meets_min() -> None:
    ...
    assert rule.validate(_commit("add feature"), _config(RuleCondition.ALWAYS, value=3)) is None


def test_subject_min_length_rule_fails_when_below_min() -> None:
    rule = SubjectMinLengthRule()
    result = rule.validate(_commit("hi"), _config(RuleCondition.ALWAYS, value=100))
    assert result is not None
...
def test_subject_max_length_rule_passes_when_within_max() -> None:
    ...
    assert rule.validate(_commit("add feature"), _config(RuleCondition.ALWAYS, value=100)) is None


def test_subject_max_length_rule_fails_when_exceeds_max() -> None:
    rule = SubjectMaxLengthRule()
    result = rule.validate(_commit("add feature"), _config(RuleCondition.ALWAYS, value=5))
    assert result is not None
