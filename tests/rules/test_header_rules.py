...
def test_header_max_length_rule_fails_when_exceeds_max() -> None:
    ...
    result = rule.validate(
        _commit("feat: this is a very long header"),
        _config(RuleCondition.ALWAYS, value=10),
    )
...
def test_header_max_length_rule_passes_at_exact_boundary() -> None:
    ...
    assert (
        rule.validate(
            _commit(header), _config(RuleCondition.ALWAYS, value=len(header))
        )
        is None
    )
...
def test_header_max_length_rule_never_condition_fails_when_within() -> None:
    rule = HeaderMaxLengthRule()
    result = rule.validate(
        _commit("feat: hi"), _config(RuleCondition.NEVER, value=100)
    )
    assert result is not None
...
def test_header_min_length_rule_passes_when_meets_min() -> None:
    ...
    assert (
        rule.validate(
            _commit("feat: header"), _config(RuleCondition.ALWAYS, value=5)
        )
        is None
    )
...
def test_header_trim_rule_passes_on_trimmed_header() -> None:
    rule = HeaderTrimRule()
    assert (
        rule.validate(_commit("feat: header"), _config(RuleCondition.ALWAYS))
        is None
    )
...
def test_header_trim_rule_fails_on_leading_space() -> None:
    rule = HeaderTrimRule()
    result = rule.validate(
        _commit(" feat: header"), _config(RuleCondition.ALWAYS)
    )
    assert result is not None
...
def test_header_trim_rule_never_condition_fails_on_trimmed() -> None:
    rule = HeaderTrimRule()
    result = rule.validate(
        _commit("feat: header"), _config(RuleCondition.NEVER)
    )
    assert result is not None
...
def test_header_full_stop_rule_passes_without_period_with_never() -> None:
    ...
    assert (
        rule.validate(
            _commit("feat: header"), _config(RuleCondition.NEVER, value=".")
        )
        is None
    )
...
def test_header_full_stop_rule_passes_with_period_and_always() -> None:
    ...
    assert (
        rule.validate(
            _commit("feat: header."), _config(RuleCondition.ALWAYS, value=".")
        )
        is None
    )
...
@pytest.mark.parametrize(
    "header,cases,condition,should_fail",
    [
...
def test_header_case_rule(
    header: str, cases: list, condition: RuleCondition, should_fail: bool
) -> None:
    rule = HeaderCaseRule()
