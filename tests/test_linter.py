...
def test_invalid_conventional_commits(
    linter: CommitLinter, message: str
) -> None:
    result = linter.lint(message)
...
def test_lint_result_has_type_case_error_for_uppercase_type(
    linter: CommitLinter,
) -> None:
...
def test_lint_result_has_type_enum_error_for_unknown_type(
    linter: CommitLinter,
) -> None:
...
def test_well_formed_commit_with_body_produces_no_warnings(
    linter: CommitLinter,
) -> None:
...
def test_lint_result_has_errors_is_false_when_no_errors(
    linter: CommitLinter,
) -> None:
...
def test_lint_result_valid_is_false_when_has_errors(
    linter: CommitLinter,
) -> None:
...
def test_factory_with_custom_config_path_accepts_valid_type(
    tmp_path: Path,
) -> None:
    config = tmp_path / ".commitlintrc.yaml"
    config.write_text("""
...
def test_factory_with_custom_config_rejects_unlisted_type(
    tmp_path: Path,
) -> None:
    config = tmp_path / ".commitlintrc.yaml"
    config.write_text("""
